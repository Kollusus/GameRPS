...
Ans.
    การซ่อน choice จะทำได้โดยการเอาตัวเลือก (01,10,11,...) มาทำการผนวกเข้ากับค่าที่สุ่มขึ้นมา และทำการ hash ค่านั้นเพื่อปกปิดค่าจริงไว้ครับ
    ```markdown
   ```solidity
    function inputPlayer(bytes32 _input) public {
        require(player_inGame[msg.sender], "You are not in this round!");
   ...
   ```
    และจะบันทึกมันไว้
    และจะทำการ reveal โดยการให้ user ใส่ค่าตัวเลือกที่ยังไม่ได้ hash เพื่อทดสอบว่าคำตอบที่ใส่มาเป็นคำตอบตั้งแต่ตอนต้นจริงๆ
    ```markdown
    ```solidity
    function reveal_choice(bytes32 _reveal) public {
